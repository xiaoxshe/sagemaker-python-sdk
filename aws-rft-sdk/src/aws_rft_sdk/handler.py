        ...
        async def async_wrapper(payload, *args, **kwargs):
            ...
            try:
                try:
                    ...
                    # Report completion and reward on success
                    try:
                        if isinstance(result, dict) and result.get("reward") is not None:
                            RolloutFeedbackClient(metadata).report_complete(result["reward"])
                        else:
                            RolloutFeedbackClient(metadata).complete_trajectory()
                    except Exception:
                        logger.exception("Failed to report rollout completion")
                    return result
            finally:
                _clear_metadata()
